...
void touch_init() {
  ...
  tp_cfg.x_max = UI_H_RES;
  tp_cfg.y_max = UI_V_RES;
...
// Physical buttons, identified live over serial (see docs/pinout.md).
//   GPIO2  = power  -> reveal the answer
...
constexpr gpio_num_t PIN_BTN_REVEAL = GPIO_NUM_2;
constexpr gpio_num_t PIN_BTN_GOOD = GPIO_NUM_40;
constexpr gpio_num_t PIN_BTN_AGAIN = GPIO_NUM_39;
...
void button_task(void*) {
  const gpio_num_t pins[] = {PIN_BTN_REVEAL, PIN_BTN_GOOD, PIN_BTN_AGAIN};
  gpio_config_t cfg = {};
  ...
  ESP_LOGI(TAG, "buttons: GPIO%d reveal, GPIO%d Good, GPIO%d Again",
           PIN_BTN_REVEAL, PIN_BTN_GOOD, PIN_BTN_AGAIN);
  ...
  while (true) {
    for (int i = 0; i < 3; ++i) {
      ...
      last[i] = level;
      if (level != 0) continue;  // act on press
      lvgl_port_lock(0);
      ESP_LOGW(TAG, "BUTTON GPIO%d pressed", pins[i]);
      if (pins[i] == PIN_BTN_REVEAL) {
        if (!g_revealed) on_reveal();
      } else if (g_revealed) {
        on_grade(pins[i] == PIN_BTN_GOOD ? 3 : 1);
      }
      lvgl_port_unlock();
    }
...
void unused_button_scan_task(void*) {
